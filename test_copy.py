...
class Test:
    ...
    def __init__(self, start, goal, finding_method, h=1, k=1, w1=1, w2=0, w3=0):
    ...
    def test(self):
        task = self.finding_method(self.start, self.goal, self.h, self.k, self.w1, self.w2, self.w3)
        task.arrange()
        print(task.path())


# class Astar_test(AStar.AStar):
#     def __init__(self, start, goal, h, k, w1, w2, w3):
#         super().__init__(start, goal, h)
#         self.k = k
#         self.w1 = w1
#         self.w2 = w2
#         self.w3 = w3
#
#     def h_cost(self):
#         return self.k * (self.w1 * self.h1 + self.w2 * self.h2 + self.w3 * self.h3)
...
class Campare:
    ...
    def run(self):
        ...
        for i in test_data:
            Astar_time_start_1 = time.time()

            task = Test(i[0], i[1], AStar_copy.AStar, 1, k=1, w1=100, w2=0, w3=0)
            task.test()

            Astar_time_end_1 = time.time()
            Astar_time_list_1.append(Astar_time_end_1 - Astar_time_start_1)

            Astar_time_start_2 = time.time()

            task = Test(i[0], i[1], AStar_copy.AStar, 1, k=1, w1=50, w2=0, w3=0)
            task.test()

            Astar_time_end_2 = time.time()
            Astar_time_list_2.append(Astar_time_end_2 - Astar_time_start_2)

        # print(Astar_time_list)
        # x = []
        # for i in range(len(Astar_time_list)):
        #     x.append(i)
        print(Astar_time_list_1)
        print(Astar_time_list_2)
        plt.scatter(Astar_time_list_1, Astar_time_list_2)
        plt.scatter(Astar_time_list_1, Astar_time_list_1)
        plt.show()
